...
| Phase 2 | trio3 三机 | ✅ 已通过（G2-T1~T6✅；2026-06-03 hover/circle/line 多跑稳定，含 Tier1 标定硬化 + Tier2 高度 re-sync + 直线终点减速 + 就绪门控）|
...
| 控制模式 | `OffboardControlMode.velocity=True`（XY 直接用 MPC 预测速度；z 轴纯 P 高度保持。1c4bc5a 起已移除 XY 的外层位置误差 P 校正——它在最优 MPC 上叠无阻尼比例环，是圆周震荡主因） |
| 坐标系 | PX4 NED（x=北, y=东, z=下）；Gazebo ENU（x=东, y=北, z=上） |
| 最终目标 | 仿真稳定后上真机（Pixhawk + 树莓派 4B），室内外均需飞行 |

> **MPC 控制输出说明**：速度控制模式。XY 直接采用 MPC 预测速度 `x_pred[1, 3:6]` 作为速度设定点（内部已含 q_pos/q_vel + 参考速度前馈 `vel+acc·t`）；z 轴用纯 P 控制器保持高度。**1c4bc5a 起移除了 XY 的外层位置误差 P 校正（原 `pred_vel + 0.5·Kp·err`, Kp=1.0）**——它在已最优的 MPC 上再叠一层无阻尼比例环，导致僚机圆周过冲震荡；移除后 trio3 圆周稳定。
...
control_hz              = 50.0    # Hz
mpc_horizon N           = 30      # 步（a77b41c：20→30，改善圆周跟踪）
mpc_dt                  = 0.05    # s，预测时域 = N×dt = 1.5 s
```
...
```python
# 圆周运动参数
leader_speed  = 0.5     # m/s，圆周/直线飞行速度（796b749 从 1.0 降为 0.5，circle 测试可显式传 1.5）
...
line_decel    = 0.5     # m/s²，终点前梯形减速度（9f6bed6，缓停防僚机过冲）

# 就绪门控（af59b66）：替代死等固定 start_delay，等全员进编队再开始运动
ready_gate_enable = True   # false 则退回固定 start_delay 旧行为
ready_pos_err     = 0.5    # m，各机 pos_err 低于此视为"进编队"
ready_hold        = 2.0    # s，就绪需连续保持时长
ready_timeout     = 90.0   # s，超时兜底：仍未就绪也开动并打红字
health_timeout    = 2.0    # s，某机 health 超此视为失联
# num_drones 由 swarm_launch 自动传入（门控需知道等几架）
...
#             (Float32MultiArray: [drone_id, status, solve_ms, fallback_cnt, hover_active, pos_err])
```
...
5. **EKF 校准时机错误**（877278d）：world_birth 校准必须等 EKF `xy_valid/z_valid` 连续 `calib_settle_frames`（25帧≈0.5s）全部有效后再锁定，避免用上电瞬态值校准导致坐标系偏移
6. **leader 不等僚机就运动**（796b749）：leader_node 起飞后先在原点悬停 `start_delay` 秒，等僚机 ARM + 爬升到编队高度后再开始运动；`leader_speed` 默认值从 1.0 改为 0.5。（注：**默认已升级为 #14 闭环就绪门控**；固定 `start_delay` 仅在 `ready_gate_enable:=false` 时作回退，`leader_start_delay` 默认 30s。）
7. **控制模式说明**：MPC 当前使用速度控制模式（`publish_velocity_setpoint`），position=NaN，velocity 为 MPC 预测速度 `x_pred[1, 3:6]`。这是正常行为，不是 bug。
8. **多机高度不一致**（5e3fdd7）：各机 EKF 启动气压计读数不同 → home 海拔(ref_alt)不同 → 同一 local_z 对应不同实际海拔（Gazebo 高度差曾达 1.9 m）。修复：`world_birth_z = birth_z + (ref_alt[0]-ref_alt[i])`，drone0 为海拔基准，z_reset 同步补偿；效果 1.91→0.05-0.28 m。**勿回退为"world_birth_z 恒=birth_z"。**
9. **圆周跟踪二阶参考**（a77b41c）：leader 输出向心加速度 ax/ay，MPC 参考轨迹改二阶 `pos+vel·t+½acc·t²`、速度前馈 `vel+acc·t`；mpc_horizon 20→30。
10. **圆周震荡——外层 P 环**（1c4bc5a）：原 XY 速度指令 = MPC 预测速度 + 0.5·Kp·位置误差(Kp=1.0)，在最优 MPC 上叠无阻尼比例环 → 僚机过冲震荡（leader 无此环故平滑）。修复：XY 直接用 `pred_vel`，z 保留纯 P。**这是 trio3 圆周稳定的决定性修复，勿回退。**
11. **标定硬化 Tier1**（30915f2）：world_birth 校准加近原点门控 `calib_max_origin_offset=2.0`（|local_xy| 仍几十米=GPS fix 前暂态，拒绝锁定）；超时不再硬锁未收敛值，改打红字 `calib STUCK` 并守出生点悬停。治 trio3 重跑「xy 塌缩」（间距→0.6m、401 碰撞）。治则 = fail loud, don't fly dirty。
12. **高度连续温漂 Tier2**（978ab9e）：#8 的 `ref_alt` 只在标定时抓一次，但 EKF ref_alt 会**连续温漂**（不触发 z_reset，旧补偿漏掉）→ 高度偏 ~0.4m。修复：drone0 广播当前 ref_alt 到 `/swarm/alt_datum`，各机限速（`alt_resync_rate=0.05`）把 `world_birth_z` 拉向 `birth_z+(datum−当前ref_alt)`；兼修 grid9 中 drone0 非邻居时的覆盖缺口。**勿回退为"只在标定时抓一次"。**
13. **直线到点过冲**（9f6bed6）：line 模式到 `max_distance` 时 vx 从 speed 直接归零（速度阶跃）→ 僚机过冲来回弹。修复：梯形速度曲线，按 `line_decel` 平滑减速到 0、发刹车加速度前馈。leader 共享 → pair2/trio3/全队形通吃。
14. **就绪门控**（af59b66）：leader 订阅各机 `health.pos_err`，**全员进编队（<`ready_pos_err` 保持 `ready_hold`）才开始运动**，`ready_timeout` 超时兜底。替代死等固定 `start_delay`，5/9 机启动免调。`ready_gate_enable:=false` 退回旧行为。
...
# 1. 清理残留进程（逐个明确进程名 + 必做验证；gz sim server 会忽略信号最易残留）
...
ps aux | grep -E "px4|gz sim|gzserver|MicroXRCE|mpc_node|leader_node|swarm_launch|ros2-daemon" | grep -v grep   # 零输出才算清干净
...
rm -rf /tmp/acados_di_mpc_*    # 仅当改了 MPC OCP 结构(horizon/状态/约束/邻居数)才清；纯 Python/参数改动不必清
...
# 注：leader 默认"就绪门控"——等全员进编队(pos_err<0.5m)才自动运动，不再死等 start_delay；
#     退回固定延时加 ready_gate_enable:=false

# 5b. 另开终端：启动诊断监控（加 --log 每秒写 CSV，供 analyze_flight.py 复盘）
python3 ~/ros2_control_mpc_ws/src/mpc_control/diag_monitor.py --formation solo1 [--log]
# 跑完出体检报告（--plot 需 matplotlib）：
python3 ~/ros2_control_mpc_ws/src/mpc_control/analyze_flight.py flight_solo1_<时间戳>.csv [--plot]
```
...
实测结果 (2026-06-02, ✅ 通过):
  修复链: start_delay 10→30s(71a97b5) + world_birth_z 用 ref_alt 差校准(5e3fdd7)
         + 圆周二阶参考 / leader 向心加速度(a77b41c) + 移除 XY 外层无阻尼 P 环(1c4bc5a)
  现象: 三机稳定跟随 leader 匀速圆周，初始暂态后收敛，无震荡/飞散
  求解: 0.17-0.32ms (<<12ms)、MPC status 全 0、高度同步
  关键: 「拆 P 环」是震荡决定性一刀（见上「MPC 控制输出说明」）

□ 三机 MPC 求解时间均 < 12 ms（trio3 每机有 2 邻居，问题规模更大）✅
□ 如求解时间 > 15 ms，记录哪架无人机并分析原因 ✅
□ 10 min 圆周运动内无 status 1/3/4 ✅（start_delay 30s + alt 校准 + 拆 P 环后稳定）
```
...
实测结果 (2026-06-02, ✅ 通过):
  现象: 高速圈(向心加速度约为 speed=0.8 时的 3.5x、约束更吃紧)三机仍稳定跟随
  无震荡、无碰撞、无飞散；MPC status 全 0
  说明: 验证拆 P 环(1c4bc5a)在更快圆周下依旧稳定，非低速侥幸
...
| Ubuntu 工作区漂移 | Ubuntu 本地有未提交调试改动时 `git pull` 失败，导致推到 GitHub 的修复根本没跑、一直测旧代码 | 每次仿真前 `git log --oneline -1`（确认 HEAD == 最新推送 commit）+ `git status`（确认工作区干净）；Ubuntu 只拉不改，修改回 Windows 改→推→拉 |
| 干净重启不彻底 | Gazebo / PX4 / ROS2 进程残留会导致旧代码状态叠加，表现为"改了没生效"或莫名飞散 | 用启动顺序步骤1 的逐进程 `pkill -9` 清理，并按其验证命令确认**零输出**（gz sim server 最易残留、需按 PID `kill -9`）再重启 |
