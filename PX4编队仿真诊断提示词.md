...
| Phase 2 | trio3 三机 | 🔄 进行中（G2-T1✅ G2-T2✅ G2-T3✅ G2-T4✅ G2-T5⏳ G2-T6⏳）|
...
6. **leader 不等僚机就运动**（796b749）：leader_node 起飞后先在原点悬停 `start_delay` 秒（默认10s），等僚机 ARM + 爬升到编队高度后再开始运动；`leader_speed` 默认值从 1.0 改为 0.5
...
| 干净重启不彻底 | Gazebo / PX4 / ROS2 进程残留会导致旧代码状态叠加，表现为"改了没生效"或莫名飞散 | 每次仿真前 `pkill -9 -f px4; pkill -9 -f gz; pkill -9 -f MicroXRCEAgent; pkill -9 -f ros2`，确认所有进程退出后再重启 |
