function res = mpc_pair2_hover()
%MPC_PAIR2_HOVER  自包含 MPC 验证：双机纵列 × 悬停
%
...
res = struct('formation','pair2','mode','hover','form_err',m_form, ...
...
title(sprintf('MPC pair2 hover — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('pair2 × hover  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
