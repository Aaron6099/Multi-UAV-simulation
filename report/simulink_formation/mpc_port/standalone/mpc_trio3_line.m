function res = mpc_trio3_line()
%MPC_TRIO3_LINE  自包含 MPC 验证：三机等边三角 × 直线
%
...
%% ── 配置 ─────────────────────────────────────────────────────────────
    ...
    cfg.mode      = 'line';
    cfg.n         = 3;
    cfg.births    = [3 0 0; -1.5 2.598 0; -1.5 -2.598 0];
    cfg.offsets   = cfg.births;
    cfg.nbrs      = {[2 3], [1 3], [1 2]};
...
res = struct('formation','trio3','mode','line','form_err',m_form, ...
...
title(sprintf('MPC trio3 line — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('trio3 × line  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
