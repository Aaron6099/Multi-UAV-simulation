function res = mpc_solo1_line()
%MPC_SOLO1_LINE  自包含 MPC 验证：单机 × 直线
%
...
res = struct('formation','solo1','mode','line','form_err',m_form, ...
...
title(sprintf('MPC solo1 line — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('solo1 × line  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
