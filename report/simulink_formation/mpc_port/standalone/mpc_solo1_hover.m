function res = mpc_solo1_hover()
%MPC_SOLO1_HOVER  自包含 MPC 验证：单机 × 悬停
%
...
res = struct('formation','solo1','mode','hover','form_err',m_form, ...
...
title(sprintf('MPC solo1 hover — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('solo1 × hover  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
