function res = mpc_trio3_circle()
%MPC_TRIO3_CIRCLE  自包含 MPC 验证：三机等边三角 × 圆周
%
...
% 队形 : trio3（三机等边三角，3 机）
% 运动 : circle（圆周）
% 时长 : T = 65 s（含收敛段 ~15s + 稳态圆周评估 ~42s），成型等待 t_start = 10 s
...
fprintf('=== trio3 × circle  （3 机, T=65s）===\n');
tic
for k = 1:Nsteps
    t = (k-1) * dt;
...
res = struct('formation','trio3','mode','circle','form_err',m_form, ...
...
title(sprintf('MPC trio3 circle — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('trio3 × circle  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
