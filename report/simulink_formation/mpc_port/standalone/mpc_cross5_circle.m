function res = mpc_cross5_circle()
%MPC_CROSS5_CIRCLE  自包含 MPC 验证：五机十字 × 圆周
%
...
fprintf('=== cross5 × circle  （5 机, T=100s）===\n');
tic
for k = 1:Nsteps
    t = (k-1) * dt;
...
res = struct('formation','cross5','mode','circle','form_err',m_form, ...
...
title(sprintf('MPC cross5 circle — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('cross5 × circle  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
