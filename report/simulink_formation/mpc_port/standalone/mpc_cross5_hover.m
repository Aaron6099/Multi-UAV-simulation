function res = mpc_cross5_hover()
%MPC_CROSS5_HOVER  自包含 MPC 验证：五机十字 × 悬停
%
...
fprintf('=== cross5 × hover  （5 机, T=30s）===\n');
tic
for k = 1:Nsteps
    t = (k-1) * dt;
...
res = struct('formation','cross5','mode','hover','form_err',m_form, ...
...
title(sprintf('MPC cross5 hover — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('cross5 × hover  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
