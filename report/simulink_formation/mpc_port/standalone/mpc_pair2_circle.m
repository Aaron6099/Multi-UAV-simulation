function res = mpc_pair2_circle()
%MPC_PAIR2_CIRCLE  自包含 MPC 验证：双机纵列 × 圆周
%
...
fprintf('=== pair2 × circle  （2 机, T=65s）===\n');
tic
for k = 1:Nsteps
    t = (k-1) * dt;
...
res = struct('formation','pair2','mode','circle','form_err',m_form, ...
...
title(sprintf('MPC pair2 circle — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('pair2 × circle  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
