function res = mpc_cross5_line()
%MPC_CROSS5_LINE  自包含 MPC 验证：五机十字 × 直线
%
...
% 队形 : cross5（五机十字，5 机）
% 运动 : line（直线）
% 时长 : T = 50 s，成型等待 t_start = 10 s
% cfg.lead_v   = 1.0;     % 领队速度 m/s
% cfg.lead_d   = 20.0;    % 直线距离 m
% cfg.lead_dec = 0.5;   % 减速度 m/s²
%
% 预期结果（对照 results.csv）：
% 预期: PASS（队形+间距+高度全满足，track_err<0.5m）
%
% ── 被控对象 ──────────────────────────────────────────────────────────
...
%% ── 配置 ─────────────────────────────────────────────────────────────
    cfg.formation = 'cross5';
    cfg.mode      = 'line';
    cfg.n         = 5;
    cfg.births    = [0 0 0; 0 3 0; 0 -3 0; 3 0 0; -3 0 0];
    cfg.offsets   = cfg.births;
    cfg.nbrs      = {[2 3 4 5], [1], [1], [1], [1]};
...
res = struct('formation','cross5','mode','line','form_err',m_form, ...
...
title(sprintf('MPC cross5 line — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('cross5 × line  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
