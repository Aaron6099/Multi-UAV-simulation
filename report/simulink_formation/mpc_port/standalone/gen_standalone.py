"""生成 12 个独立 standalone MATLAB 文件（每个场景一个）"""
import os
...
FORMATIONS = {
    ...
    'trio3': {
        ...
        'births_rows': ['[3 0 0]', '[-1.5 2.598 0]', '[-1.5 -2.598 0]'],
        'nbrs': "{[2 3], [1 3], [1 2]}",
    },
    'cross5': {
        'label': '五机十字',
        ...
        'births_rows': ['[0 0 0]', '[0 3 0]', '[0 -3 0]', '[3 0 0]', '[-3 0 0]'],
        'nbrs': "{[2 3 4 5], [1], [1], [1], [1]}",
    },
}

MODES = {
    'hover':  {'label': '悬停',   'T': 30,  't_start': 10},
    'line':   {'label': '直线',   'T': 50,  't_start': 10, 'lead_v': 1.5, 'lead_d': 20.0, 'lead_dec': 0.5},
    'circle': {'label': '圆周',   'T': 65,  't_start': 10, 'lead_R': 10.0, 'lead_v': 1.5},
}
...
# ── 文件模板生成 ────────────────────────────────────────────────────────────
def make_file(fname, fid, mid, fcfg, mcfg, overrides):
    n = fcfg['n']
    flabel = fcfg['label']
    mlabel = mcfg['label']
    T = overrides.get('T', mcfg['T'])
    t_start = overrides.get('t_start', mcfg['t_start'])

    # 场景参数注释块
    mode_params = []
    if mid == 'line':
        lead_v = overrides.get('lead_v', mcfg['lead_v'])
        mode_params += [
            f"% cfg.lead_v   = {lead_v};     % 领队速度 m/s",
            f"% cfg.lead_d   = {mcfg['lead_d']};    % 直线距离 m",
            f"% cfg.lead_dec = {mcfg['lead_dec']};   % 减速度 m/s²",
        ]
    elif mid == 'circle':
        lead_v = overrides.get('lead_v', mcfg['lead_v'])
        w_coll = overrides.get('w_coll', None)
        mode_params += [
            f"% cfg.lead_R = {mcfg['lead_R']};      % 圆半径 m",
            f"% cfg.lead_v = {lead_v};      % 圆周速度 m/s  → 周期≈{2*3.14159*mcfg['lead_R']/lead_v:.0f}s",
        ]
        if w_coll:
            mode_params += [f"% cfg.w_coll = {w_coll};    % 碰撞权重（cross5 circle 特殊）"]
    ...
    # 出生位置代码
    birth_lines = []
    for i, b in enumerate(fcfg['births_rows']):
        birth_lines.append(f"pos(:,{i+1}) = {b}';")
    ...
    cfg_lines = [
        ...
        f"cfg.mode      = '{mid}';",
        f"cfg.n         = {n};",
        f"cfg.births    = {fcfg['births']};",
        f"cfg.offsets   = cfg.births;",
        f"cfg.nbrs      = {fcfg['nbrs']};",
        "",
        "% 期望间距 d*（xy 平面）",
        ...
        "cfg.q_term_s   =  2.0;",
        "cfg.d_safe     =  1.5;",
        f"cfg.w_coll     = {overrides.get('w_coll', 200.0)};",
        "cfg.w_form     =  0.5;",
        "cfg.lm         =  1e-4;",
        "cfg.kp_z       =  1.0;",
        f"cfg.t_start    = {float(t_start)};",
        f"cfg.T          = {T};",
    ]
    ...
    if mid == 'line':
        ...
        cfg_lines += [
            f"cfg.lead_v   = {lead_v};",
            f"cfg.lead_d   = {mcfg['lead_d']};",
            f"cfg.lead_dec = {mcfg['lead_dec']};",
        ]
    elif mid == 'circle':
        lead_v = overrides.get('lead_v', mcfg['lead_v'])
        cfg_lines += [
            f"cfg.lead_R = {mcfg['lead_R']};",
            f"cfg.lead_v = {lead_v};",
        ]

    cfg_code = '\n'.join('    ' + l if l else '' for l in cfg_lines)
    birth_code = '\n'.join('    ' + l for l in birth_lines)

    verdict_note = (
        "% REVIEW = 共模速度滞后（队形完好、间距安全、整体均匀落后参考）= 非编队缺陷"
        if mid in ('line', 'circle') and fid != 'cross5'
        else "% 预期: PASS（队形+间距+高度全满足，track_err<0.5m）"
    )

    content = f"""function res = {fname}()
%{fname.upper()}  自包含 MPC 验证：{flabel} × {mlabel}
%
...
% 队形 : {fid}（{flabel}，{n} 机）
% 运动 : {mid}（{mlabel}）
% 时长 : T = {T} s，成型等待 t_start = {t_start} s
{mode_block}% 预期结果（对照 results.csv）：
{verdict_note}
%
% ── 被控对象 ──────────────────────────────────────────────────────────
...
res = struct('formation','{fid}','mode','{mid}','form_err',m_form, ...
...
title(sprintf('MPC {fid} {mid} — 俯视轨迹'));
...
xlabel('t [s]');
sgtitle(sprintf('{fid} × {mid}  →  %s  (form=%.3fm  track=%.3fm  minsp=%.2fm)', ...
...
# ── 生成全部 12 个文件 ───────────────────────────────────────────────────────
count = 0
for fid, fcfg in FORMATIONS.items():
    for mid, mcfg in MODES.items():
        overrides = CROSS5_OVERRIDES.get(mid, {}) if fid == 'cross5' else {}
        fname = f"mpc_{fid}_{mid}"
        path  = os.path.join(OUTDIR, fname + '.m')
        code  = make_file(fname, fid, mid, fcfg, mcfg, overrides)
        with open(path, 'w', encoding='utf-8') as f:
            f.write(code)
        print(f"  写入: {fname}.m")
        count += 1

print(f"\n完成：共生成 {count} 个文件 → {OUTDIR}")
