...
BEFORE the offboard mode switch command is sent. So this node waits a few
seconds (during which flock_controller_node should already be streaming
offboard_control_mode + trajectory_setpoint) before issuing the arm command.

The recommended startup sequence is:
  1. Start all 9 PX4 SITL instances
  2. Start MicroXRCEAgent
  3. Start virtual_leader_node and 9 flock_controller_nodes
     (these immediately start streaming TrajectorySetpoint at 20Hz)
  4. Wait until flock_controllers report "ready" (or wait fixed setup_seconds)
  5. Start this arming_node — it sends mode + arm commands
    ...
    def tick(self):
        ...
        if self.state == 'WAITING':
            if self.tick_count >= self.setup_ticks:
                self.get_logger().info('Setup wait complete. Sending OFFBOARD mode commands.')
                self.state = 'SET_MODE'
